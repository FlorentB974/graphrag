...
def create_plotly_graph(graph_data: Dict[str, Any], layout_algorithm: str = 'spring') -> go.Figure:
    ...
    node_trace = go.Scatter(
        x=x_coords, y=y_coords, text=text_labels, textposition='middle center',
...
def get_query_graph_data(query_results: List[Dict[str, Any]], query_text: str = "Your Question") -> Dict[str, Any]:
    """
    ...
    for i, result in enumerate(query_results):
        ...
        display_content = chunk_content[:80] + "..." if len(chunk_content) > 80 else chunk_content
            ...
            if doc1 == doc2 and doc1:  # Same document relationship
                ...
                if words1 and words2:
                    ...
                    if overlap >= 3:  # Minimum word overlap threshold
                        ...
                        if similarity_score > 0.15:  # Slightly higher threshold for cross-doc relations
                            edges.append({
                                'source': f"chunk_{i}",
                                'target': f"chunk_{j}",
...
def create_query_result_graph(query_results: List[Dict[str, Any]], query_text: str = "Your Question") -> go.Figure:
    """
