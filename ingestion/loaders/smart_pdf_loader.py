"""
Enhanced PDF loader with smart OCR integration.
...
class SmartPDFLoader:
    ...
    def load(self, file_path: Path) -> Optional[str]:
        """
        Load text content from a PDF file with smart OCR.
    ...
    def load_with_metadata(self, file_path: Path) -> Optional[Dict[str, Any]]:
        """
        Load PDF content with smart OCR and detailed metadata.
        ...
        """
        try:
            logger.info(f"Processing PDF with smart OCR: {file_path}")
            
            # Use smart OCR processor to intelligently handle the PDF
            ...
            logger.info(
                f"Smart PDF processing completed for {file_path}: "
            ...
            logger.error(f"Failed to load PDF with smart OCR {file_path}: {e}")
            return None

    def load_with_ocr(self, file_path: Path, enable_ocr: bool = True) -> Optional[str]:
        """
        Legacy method for compatibility.
        Now uses smart OCR regardless of enable_ocr parameter.

        Args:
            file_path: Path to the PDF file
            enable_ocr: Ignored (smart OCR always applies intelligently)
        ...
        if not enable_ocr:
            ...
            # Use smart OCR (default behavior)
