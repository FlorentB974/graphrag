...
class PPTXLoader:
    ...
    def _process_slide(self, slide, slide_number: int) -> str:
        ...
        for shape_idx, shape in enumerate(slide.shapes):
            try:
                ...
                if shape.shape_type == MSO_SHAPE_TYPE.TABLE:
                    ...
                    content_parts.append(f"[Image: {getattr(shape, 'name', f'Image_{shape_idx}')}]")
                
                elif shape.shape_type == MSO_SHAPE_TYPE.CHART:
                    # Note presence of chart
                    content_parts.append(f"[Chart: {getattr(shape, 'name', f'Chart_{shape_idx}')}]")
                    ...
                    if text_content:
                        ...
                        is_title = (
                            not title_found and
                            (hasattr(shape, 'name') and 'title' in shape.name.lower()) or
                            len(text_content) < 100  # Short text likely to be title
                        )
                ...
                logger.debug(f"Error processing shape {shape_idx} on slide {slide_number}: {e}")
                continue
        ...
        if len(full_slide_content) > self.max_slide_content_length:
            full_slide_content = full_slide_content[:self.max_slide_content_length] + "\n[Content truncated...]"
    ...
    def _generate_presentation_summary(self, metadata: Dict[str, Any], filename: str) -> str:
        """
        ...
        summary.append(f"  - Slides with text content: {metadata['slides_with_content']}")
        ...
        content_ratio = metadata['slides_with_content'] / metadata['slide_count'] if metadata['slide_count'] > 0 else 0
    ...
    def load(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            presentation_summary = self._generate_presentation_summary(metadata, file_path.name)
            ...
            slide_contents.append(self._generate_content_insights(slide_contents, metadata))
    ...
    def _generate_content_insights(self, slide_contents: List[str], metadata: Dict[str, Any]) -> str:
        """
        ...
        business_keywords = ['revenue', 'profit', 'growth', 'market', 'sales', 'customer', 'strategy', 'roi', 'kpi']
        technical_keywords = ['algorithm', 'data', 'system', 'architecture', 'implementation', 'api', 'database']
        educational_keywords = ['learn', 'understand', 'example', 'definition', 'concept', 'theory', 'practice']
        
        business_score = sum(1 for keyword in business_keywords if keyword in all_text)
        technical_score = sum(1 for keyword in technical_keywords if keyword in all_text)
        ...
        # Content complexity
        avg_shapes_per_slide = metadata['total_shapes'] / metadata['slide_count'] if metadata['slide_count'] > 0 else 0
        ...
        if metadata['slides_with_tables'] > 0 and metadata['slides_with_charts'] > 0:
            insights.append("Structure: Data-driven presentation with both tables and visualizations")
