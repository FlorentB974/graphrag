...
def ingest_single_file(file_path: Path) -> bool:
    ...
    try:
        ...
        if result and result.get("status") == "success":
            ...
            logger.info(f"✅ Successfully processed {file_path}: {chunks_created} chunks created")
            return True
        else:
...
def ingest_directory(directory_path: Path, recursive: bool = False) -> tuple[int, int]:
    ...
    try:
        ...
        logger.info(f"✅ Directory processing complete: {successful}/{total} files successful")
