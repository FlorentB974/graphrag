...
class TokenManager:
    ...
    def _get_context_size(self) -> int:
        ...
        # Default fallback
        logger.warning(f"Unknown model '{self.model}', using default context size of {self.MODEL_CONTEXT_SIZES['default']} tokens")
    ...
    def _get_encoding(self):
        ...
        try:
            ...
            logger.warning(f"Could not get tiktoken encoding for model {self.model}: {e}")
    ...
    def count_message_tokens(self, messages: List[Dict[str, str]]) -> int:
        ...
        for message in messages:
            ...
            total_tokens += role_tokens + content_tokens + 4  # 4 tokens per message overhead
    ...
    async def get_model_context_size(self, llm_manager) -> int:
        ...
        try:
            ...
            logger.warning(f"Could not determine context size from model response: {response}")
    ...
    def split_context_chunks(self,
                             query: str,
                             context_chunks: List[Dict[str, Any]],
                             system_message: Optional[str] = None) -> List[Tuple[str, List[Dict[str, Any]], int]]:
        """
        ...
        if available_tokens <= 0:
            logger.error(f"Query and system message too long: {query_tokens + system_tokens} tokens exceed limit")
        ...
        for chunk in context_chunks:
            ...
            if chunk_tokens > available_tokens:
                ...
                if current_batch:
                    batches.append((query, current_batch, current_tokens + query_tokens + system_tokens))
                    current_batch = []
                    current_tokens = 0
                
                batches.append((query, [truncated_chunk], available_tokens + query_tokens + system_tokens))
                continue
    ...
    def needs_splitting(self,
                        query: str,
                        context_chunks: List[Dict[str, Any]],
                        system_message: Optional[str] = None) -> bool:
