...
class CSVLoader:
    ...
    def _analyze_dataframe(self, df: pd.DataFrame, filename: str) -> str:
        ...
        for i, col in enumerate(df.columns):
            ...
            analysis.append(f"  - Non-null values: {non_null}/{df.shape[0]} ({non_null/df.shape[0]*100:.1f}%)")
            if null_count > 0:
        ...
        if len(numeric_cols) > 0:
            analysis.append(f"Numeric columns ({len(numeric_cols)}): {', '.join(numeric_cols[:10])}")
            for col in numeric_cols[:5]:  # Show stats for first 5 numeric columns
                try:
                    stats = df[col].describe()
                    analysis.append(f"  {col}: min={stats['min']:.2f}, max={stats['max']:.2f}, mean={stats['mean']:.2f}")
                except:
                    pass
        ...
        if len(categorical_cols) > 0:
            analysis.append(f"Text/categorical columns ({len(categorical_cols)}): {', '.join(categorical_cols[:10])}")
            for col in categorical_cols[:5]:  # Show unique counts for first 5
                ...
                if unique_count < 100:  # Only show if reasonable number
                    ...
                    analysis.append(f"  {col}: {unique_count} unique values. Top: {top_str}")
    ...
    def _generate_business_insights(self, df: pd.DataFrame) -> str:
        ...
        if date_columns:
            insights.append(f"Time-based analysis possible with columns: {', '.join(date_columns)}")
        ...
        if potential_relationships:
            insights.append(f"Potential relationship/grouping columns: {', '.join(potential_relationships)}")
        ...
        for col in df.columns:
            if (df[col].dtype in ['int64', 'float64'] and 
                any(keyword in col.lower() for keyword in 
                    ['amount', 'price', 'cost', 'revenue', 'count', 'total', 'sum', 'avg', 'rate', 'percent'])):
                metric_columns.append(col)
        
        if metric_columns:
            insights.append(f"Business metric columns identified: {', '.join(metric_columns)}")
        ...
        if completeness_issues:
            insights.append(f"Data quality attention needed: {', '.join(completeness_issues)}")
