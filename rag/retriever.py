...
class DocumentRetriever:
    ...
    def retrieve_with_graph_expansion(
        self, query: str, top_k: int = 3, expand_depth: int = 2
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"Expanded retrieval: {len(initial_chunks)} -> {len(expanded_chunks)} chunks"
            )
    ...
    def hybrid_retrieval(
        self, query: str, doc_filter: List[str] = None, top_k: int = 5  # type: ignore
    ) -> List[Dict[str, Any]]:
        """
